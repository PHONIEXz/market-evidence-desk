...
- [x] The published dataset contains 101 research questions and 22 source records (21 distinct source URLs), as verified by an anonymous published query on 2026-09-25. The Sourcebook rebuild was reported complete; its final indexed count, entry coverage, and issue review still require verification.
- [ ] Publish or promote the tested build to a public URL and recheck the ready state there. The current feature-preview URL redirects unauthenticated visitors to Vercel sign-in, while `market-evidence-desk.vercel.app` responds publicly but served a build last modified 2026-09-23 when checked on 2026-09-25. Update the DEV demo links only after verifying the public build.
